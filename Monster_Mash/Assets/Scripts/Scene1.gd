...
func build_level():
	...
	for x in range(level_size.x):
		...
		for y in range(level_size.y):
			map[x].append(Tile.Stone)  # stone is a placeholder for 
...
func connect_rooms():
	...
	for room in rooms:
		...
		room_graph.add_point(point_id, Vector3(room_center.x, room.center.y, 0))
		point_id += 1
...
func cut_regions(free_regions, region_to_remove):
	...
	for region in free_regions: 
		if region.intersects(region_to_remove):
			...
			var leftover_below = region.end.y - region_to_remove.y - 1
